class Laser {
  ...
  create(container) {
    ...
    if(LASER_LEVELS == 1) {
      $element.src = "Images/laser-level1.png";
      $element.className = "laser-1";
    }
    else if(LASER_LEVELS == 2) {
      $element.src = "Images/laser-level2.png";
      $element.className = "laser-2";
    }
    else if(LASER_LEVELS >= 3) {
      $element.src = "Images/laser-level3.png";
      $element.className = "laser-3";
    }
...
function updateEnemyLasers(dt, containers) {
  ...
  for (let i = 0; i < lasers.length; i++) {
    const laser = lasers[i];
    laser.y += dt * LASER_ENEMY_MAX_SPEED;
    ...
    if (rectsIntersect(r1, r2)) {
      destroylasers(containers, laser);
      lives--;
      if(LASER_LEVELS > 1 ) LASER_LEVELS --;
      else LASER_COOLDOWN += 0.02;
